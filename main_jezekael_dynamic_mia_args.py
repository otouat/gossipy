...
def parse_args():
    ...
    parser.add_argument(
        "--architecture", type=str, default="ResNet20", help="Model architecture"
    )
    ...
    parser.add_argument(
        "--neighbors", type=int, default=2, help="Number of neighbors in the network"
    )
    parser.add_argument(
        "--test_size", type=float, default=0.5, help="Test set size fraction"
    )
    parser.add_argument(
        "--factors",
        type=int,
        default=1,
        help="Factor to multiply nodes count for the dispatcher",
    )
    parser.add_argument(
        "--beta", type=float, default=0.75, help="Class distribution parameter"
    )
    parser.add_argument(
...
def main():
    ...
    message += f" | Training: {args.n_nodes} nodes, {args.n_local_epochs} local epochs, batch size {args.batch_size}, neighbors {args.neighbors}, peer sampling period: {args.peer_sampling_period}"
    ...
    data_handler = ClassificationDataHandler(
        Xtr, ytr, Xte, yte, test_size=args.test_size
    )

    # Data dispatcher
    data_dispatcher = OLDCustomDataDispatcher(
        data_handler, n=args.n_nodes * args.factors, eval_on_user=True, auto_assign=True
    )
    ...
    topology = UniformDynamicP2PNetwork(
        ...
        topology=nx.to_numpy_array(
            random_regular_graph(args.neighbors, args.n_nodes, seed=42)
        ),
    )
