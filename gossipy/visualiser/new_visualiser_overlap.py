...
df1 = pd.read_csv(r"C:\Users\jezek\OneDrive\Documents\Python\Djack\gossipy\results\Exp_n#190\mia_results.csv")
df2 = pd.read_csv(r"C:\Users\jezek\OneDrive\Documents\Python\Djack\gossipy\results\Exp_n#191\mia_results.csv")  # Update the path for the second experiment
...
axs[0, 0].plot(rounds1, avg_train_acc1, 'C1:', label='Exp 1: Accuracy on train set')
axs[0, 0].fill_between(rounds1, avg_train_acc1 - std_train_acc1, avg_train_acc1 + std_train_acc1, color='C1', alpha=0.2)
axs[0, 0].plot(rounds1, avg_local_test_acc1, 'C1-', label='Exp 1: Accuracy on local test set')
axs[0, 0].fill_between(rounds1, avg_local_test_acc1 - std_local_test_acc1, avg_local_test_acc1 + std_local_test_acc1, color='C2', alpha=0.2)
axs[0, 0].plot(rounds1, avg_global_test_acc1, 'C1--', label='Exp 1: Accuracy on global test set')
...
axs[0, 0].plot(rounds2, avg_train_acc2, 'C2:', label='Exp 2: Accuracy on train set')
axs[0, 0].fill_between(rounds2, avg_train_acc2 - std_train_acc2, avg_train_acc2 + std_train_acc2, color='C1', alpha=0.1)
axs[0, 0].plot(rounds2, avg_local_test_acc2, 'C2-', label='Exp 2: Accuracy on local test set')
axs[0, 0].fill_between(rounds2, avg_local_test_acc2 - std_local_test_acc2, avg_local_test_acc2 + std_local_test_acc2, color='C2', alpha=0.1)
axs[0, 0].plot(rounds2, avg_global_test_acc2, 'C2--', label='Exp 2: Accuracy on global test set')
...
axs[0, 1].plot(rounds1, gen_errors1, 'C1-', label='Exp 1: Generalization Error')
axs[0, 1].plot(rounds2, gen_errors2, 'C2-', label='Exp 2: Generalization Error')
...
avg_mar_mia_entropy1 = df1.groupby('Round')['marginalized_mia_entropy'].mean()
...
avg_mar_mia_entropy2 = df2.groupby('Round')['marginalized_mia_entropy'].mean()
...
axs[1, 0].scatter(avg_global_test_acc1, avg_mia_entropy1, label='Exp 1: Average MIA Entropy', color='C1')
axs[1, 0].scatter(avg_global_test_acc1, avg_mar_mia_entropy1, label='Exp 1: Average Marginalized MIA Entropy', color = 'C3', marker='x')
axs[1, 0].scatter(avg_global_test_acc2, avg_mia_entropy2, label='Exp 2: Average MIA Entropy', color='C2')
axs[1, 0].scatter(avg_global_test_acc2, avg_mar_mia_entropy2, label='Exp 2: Average Marginalized MIA Entropy', color = 'C4', marker='x')

axs[1, 0].set_xlabel('Global Test Accuracy')
axs[1, 0].set_ylabel('Average MIA Vulnerability')
axs[1, 0].set_title('Average MIA Vulnerability over Global Test Accuracy')
axs[1, 0].legend()
axs[1, 0].grid(True)

# Calculating the average MIA vulnerability over the generalization errors for both experiments
axs[1, 1].scatter(gen_errors1, avg_mia_entropy1, label='Exp 1: Average MIA Entropy', color='C1')
axs[1, 1].scatter(gen_errors1, avg_mar_mia_entropy1, label='Exp 1: Average Marginalized MIA Entropy', color='C3', marker='x')
axs[1, 1].scatter(gen_errors2, avg_mia_entropy2, label='Exp 2: Average MIA Entropy', color='C2')
axs[1, 1].scatter(gen_errors2, avg_mar_mia_entropy2, label='Exp 2: Average Marginalized MIA Entropy', color='C4', marker='x')
